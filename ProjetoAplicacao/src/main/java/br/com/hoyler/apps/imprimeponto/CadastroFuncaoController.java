...
public class CadastroFuncaoController {
	...
	void ButtonAlterarID_OnAction(ActionEvent event) {
		...
		if (BOTAO.equals(ATUALIZAR)) {
			...
			if (TextFieldNomeID.getText().length() >= 1) {
				...
				if (resportaPergunta) {
					...
					funcoesFabricaDAO.UpdateFuncoesDados(CadastroAntigo, NOME);
					...
					new CriarMensagens().MensagemInformacoes("Atualizado com SUCESSO", NOME);
		...
		if (BOTAO.equals(CONFIRMAR)) {
			...
			if (TextFieldCodigoID.getText().length() >= 1) {
				...
				if (resportaPergunta) {
					...
					retornoBool = new FuncoesFabricaDAO().DeletarCodigosTabelaFuncoes(CODIGO);
		...
		if (BOTAO.equals(SALVAR)) {
			...
			if (TextFieldNomeID.getText().length() >= 1) {
				...
				new FuncoesFabricaDAO().SalvarFuncoesDados(NOME);
				...
				TextFieldListarID.setDisable(false);
				TableViewColaboradorID.setDisable(false);
				new CriarMensagens().MensagemInformacoes("Salvo com SUCESSO", NOME);
				cadastroFuncao.setFocusTextField(TextFieldListarID);
			} else {
	...
	void ButtonImprimirID_OnAction(ActionEvent event) {
		...
		if (resportaPergunta) {
			...
			if (NOME.isEmpty()) {
				...
				jrResultSetDataSource = new JRResultSetDataSource(funcoesFabricaDAO.ListarFuncoes(null));
				...
				ProgressBarID.setProgress(1);
				ProgressBarID.setVisible(true);
				jrResultSetDataSource = new JRResultSetDataSource(funcoesFabricaDAO.ListarFuncoes(NOME));
		...
		if (NOME.isEmpty()) {
			...
			if (file != null) {
				...
				try {
					...
					System.out.println(
							"public class CadastroFuncaoController ButtonPDFID_OnAction Impressao [ERRO] 1\n\n"
									+ e.getMessage() + "\n\n");

				} catch (IOException e) {

					System.out.println(
							"public class CadastroFuncaoController ButtonPDFID_OnAction Impressao [ERRO] 2\n\n"
									+ e.getMessage() + "\n\n");
	...
	void ButtonVisualizarID_OnAction(ActionEvent event) {
		...
		if (NOME.isEmpty()) {
			...
			try {
				...
				jrResultSetDataSource = new JRResultSetDataSource(funcoesFabricaDAO.ListarFuncoes(null));
				ProgressBarID.setProgress(25);
				jasperReport = (JasperReport) JRLoader.loadObject(new br.com.hoyler.apps.reports.Relatorios().getRelatorioImprimeFuncoes());

				jasperPrint = JasperFillManager.fillReport(jasperReport, null, jrResultSetDataSource);
				ProgressBarID.setProgress(75);
				
			 
				new JasperViewerFX((Stage) vBox.getScene().getWindow()).viewReport("Relatorios Imprime Funções", jasperPrint);
    ...
    void initialize() {
		...
		TableColumnCodigoID.setCellValueFactory(new PropertyValueFactory<Funcoes, Integer>("CODIGO"));
		TableColumnFuncaoID.setCellValueFactory(new PropertyValueFactory<Funcoes, String>("NOME"));
		// TableViewColaboradorID.setItems(new br.com.hoyler.apps.database.sqlite().ListarFuncoesNomes(""));
